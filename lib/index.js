...
function errMark(lines, line, col, msg, src) {
  var sep = (new Array(Math.max(col + 13, 20 - col))).join('-');

  lines.splice(line, 0, [
    sep.substr(0, col) + '^' + sep.substr(col, sep.length - col - 1),
    msg + '\nat <' + src + ':' + line + '>',
    sep
  ].join('\n'));
...
function fixedError(err) {
  // jade
  if (err && err.extract) {
    return errMark(err.extract, err.line + 1, err.column, err.message.trim(), err.filename);
  }
  ...
  // coffee-script
  if (err && err.location) {
    return errMark(err.code.split('\n'), err.location.first_line + 1, err.location.first_column, err.message.trim(), err.filename);
  }
}
