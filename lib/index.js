...
module.exports = function(options, done) {
  ...
  if (options.server) {
    require('./browser-sync').call(context, options);
  }
  ...
  if (options.require) {
    $.toArray(options.require).forEach(function(file) {
      ...
      _.then(function(next) {
        if (cb.length === 2) {
          cb.call(context, options, next);
        } else {
          cb.call(context, options);
          next();
        }
      });
    });
  ...
  return readFiles.call(context, options, deps, function(err1, result) {
    function end(err) {
      if (options.cache) {
    ...
    function build(err2) {
      ...
      if (!err2) {
        compileFiles.call(context, options, result, end);
      } else {
        end(err2);
      }
    }
