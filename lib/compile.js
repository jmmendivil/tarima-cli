...
module.exports = function compileFiles(result, cb) {
  ...
  function ensureWrite(view, index) {
    // TODO: configure this keyword?
    if (typeof view.data._render === 'string') {
      var template = path.resolve(path.dirname(view.src), view.data._render),
          partial = tarima.load(template, tarimaOpts());
      ...
      $.merge(locals, { yield: view.output });

      view.output = partial.data(locals, partial.params.ext === 'js').source;
    }
