...
module.exports = function(result, done) {
  ...
  function bundle(file) {
    return log(file, function(next) {
      ...
      // TODO: allow overrides for everything?
      $.merge(opts, options.bundleOptions);
      $.merge(opts, bundleOptions);

      if (typeof data[BUNDLE_KEY] === 'object') {
        $.merge(opts, {
          standalone: data[BUNDLE_KEY].name,
          bundleExternal: data[BUNDLE_KEY].external,
          externalRequireName: data[BUNDLE_KEY].require
        });
