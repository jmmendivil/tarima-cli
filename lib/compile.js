...
module.exports = function(result, done) {
  ...
  function compile(partial, next) {
    console.log('BUNDLE!!!', partial);
    next();
  }

  function append(partial, next) {
    partial.render(function(err, data) {
      if (!err) {
        var file = data.src,
            target = dest(data.id, data.ext);
        ...
        if (tarima.support.isScript(data.ext) && reTemplateFunction.test(data.code)) {
  ...
  result.files.forEach(function(src) {
    if (!tarima.support.isSupported(src)) {
      ...
      if (options.bundle && tarima.support.isScript(partial.params.ext)) {
